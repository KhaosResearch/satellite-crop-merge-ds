...
- The Satellite Crop and Merge Downloader is an agrotech application designed for the EDAAn Data Space. It allows users to acquire different satellite product in the form of a compressed collection of TIF files. Data is cut from the geometry input the user provides. The application takes the specified date range, takes the images from the source (MinIO or Sentinel Hub) and performs the Crop & Merge operations.
...
- **Date range:** Date inputs to specify the temporal coverage.
  - The dates must be in ISO format (YYYY-MM-DD).
  - MinIO takes only YYYY and MM to retrieve data since the database is made of monthly composites.
  - Sentinel Hub does need the exact date to calculate time coverage and retrieve exact data.
...
2. **Geometry GeoJSON file**:
   - Optional download file. It contains the geometry data used as a GeoJSON file. 
